...
function SettingsPage({ activePage, setActivePage }) {
  ...
  class SettingsButton {
    constructor({ icon, title, subtitle, action }) {
      this.icon = icon;
      this.title = title;
  ...
  const buttonList = [
    new SettingsButton({ icon: '', title: 'Akun', subtitle: 'Notifikasi keamanan, ganti nomor', action: () => { setActiveSettingsPage('account') }}),
    new SettingsButton({ icon: '', title: 'Privasi', subtitle: 'Akun diblokir, pesan sementara', action: () => {}}),
    new SettingsButton({ icon: '', title: 'Daftar', subtitle: 'Kelola orang dan grup', action: () => {}}),
    new SettingsButton({ icon: '', title: 'Chat', subtitle: 'Tema, wallpaper, riwayat obrolan', action: () => {}}),
    new SettingsButton({ icon: '', title: 'Tampilan', subtitle: 'Tema obrolan, ikon aplikasi, tema aplikasi', action: () => {}}),
  ]
  return (
    <>
      { activeSettingsPage == 'account' && ( <AccountPage activeSettingsPage={activeSettingsPage} setActiveSettingsPage={setActiveSettingsPage} />)}
      { !activeSettingsPage && (
              ...
              <ul style={{ marginTop: '44px', listStyle: 'none', width: '100%' }}>
                {
                  buttonList.map( item => (
                    <li style={{ display: 'flex', width: '100%', height: 'auto' }}>
                      <button onClick={item.action} style={{ paddingInline: '12px', gap: '16px', display: 'flex', width: '100%', alignItems: 'center', backgroundColor: 'transparent', border: 'none', paddingBlock: '12px' }}>
